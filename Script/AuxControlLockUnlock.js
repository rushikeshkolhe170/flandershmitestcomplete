...
const {wpfControlText, searchAndSelectCheckboxComm, visibilityInBooleanStatus, clickAndHoldButton, criticalMsgOk, logIn, checkVisibilityAndClick, verifyWpfControlTextWithExistCheck, verifyVisibilityWithExistCheck} = require('CommonFunc');

function AuxControlLockUnlock()
{
  ...
  var icon1 = wpfControlText(auxControlVariables.tabs.Child(3).Child(4).Child(0).Child(0).Child(8).Child(0).Child(0));
  if(icon1 == "UNLOCKED")
  {
    Log.Error("Pin did not locked.");
  }
  ...
  var icon = wpfControlText(auxControlVariables.tabs.Child(3).Child(4).Child(0).Child(0).Child(8).Child(0).Child(0));
  if(icon == "LOCKED")
  {
    Log.Error("Pin did not unlocked.");
  }
